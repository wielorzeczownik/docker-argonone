...
do
	...
	then
		...
		do
			echo -n "Please provide fan speed (0-100):"

			curfan=$( get_number )
			# PATCH: allow low duty cycles; accept 0-100
	...
	then
		...
		do
			...
			do
				...
				echo -n ""$displaytemp"$temperature (0-100):"
	...
	then
		...
		do
			...
			while [ $errorfanflag -eq 1 ]
			do
				echo -n "Provide fan speed for "$curtemp"$temperature (0-100) then [ENTER]:"
				curfan=$( get_number )
				# PATCH: allow low duty cycles; accept 0-100
				if [ $curfan -ge 0 ] && [ $curfan -le 100 ]
				then
